...
def bound_check(arr,i,j):
    ...
    if i==rows or j==cols:
        return 0
    if arr[i][j] == 1:#my piece
        return 1
    else:
        return 0

def one_radius(arr,i,j):
    count = 0
    spaces = [[0,1],[1,1],[1,0],[1,-1]]#right right_down down left_down
    for space in spaces:
        if bound_check(arr,i+space[0],j+space[1]):
            count+= 1
    return count
        
def two_radius(arr,i,j):
    count = 0
    spaces = [[0,1],[1,1],[1,0],[1,-1]]
    #spaces = [[[0,1],[0,2]],[[1,1],[2,2]],[[1,0],[2,0]],[[0,-1],[0,-2]]]
    for space in spaces:
        if bound_check(arr,i+space[0],j+space[1]) and bound_check(arr,i+2*space[0],j=2*space[1]):
            count+=1
    return count
